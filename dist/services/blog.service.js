...
async function getAllBlogPub(skip, limit) {
    ...
    const { error } = validation_1.default.blogJoi.getAllBlogPub.validate({
        skip,
        limit,
    });
...
async function create(createReq, userId) {
    const { error } = validation_1.default.blogJoi.createBlog.validate({
        createReq,
        userId,
    });
    if (error) {
...
async function checkAdmin(userName, password) {
    // Here use joi library to validate value
    const { error } = validation_1.default.blogJoi.checkAdmin.validate({
        userName,
        password,
    });
    // Throwing a validation error
